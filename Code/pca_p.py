...
data = pd.read_csv('/Volumes/CAS9/Aeiwz/Documents/Thesis/Analyse/Dataset/P_cpmg_no_pqn.csv')
...
for i in range(len(dataset)):
    ...
    df2_scores_['Plasma source'] = meta['Plasma source']
    ...
    fig = px.scatter(df2_scores_, x='PC1', y='PC2', symbol=meta['Plasma source'],
...
# Loading plot
    ...
    traject_df['Plasma source'] = meta['Plasma source']
    med_df = traject_df.groupby(['Plasma source', Group]).median()
    err_df = traject_df.groupby(['Plasma source', Group]).sem()
    
    med_df = med_df.reset_index()
    err_df = err_df.reset_index()
    
    
    med_df.sort_values(by=['Plasma source'], inplace=True, key=lambda x: x.map({"1-wk pre-op": 0, "1-wk post-op": 1, "2-wk post-op": 2, "4-wk post-op": 3}))
    med_df.sort_values(by=[Group], inplace=True)
    med_df.reset_index(drop=True, inplace=True)
    
    err_df.sort_values(by=['Plasma source'], inplace=True, key=lambda x: x.map({"1-wk pre-op": 0, "1-wk post-op": 1, "2-wk post-op": 2, "4-wk post-op": 3}))
    ...
    fig = px.line(med_df, x='PC1', y='PC2', line_group='Plasma source', error_x=err_df["PC1"], error_y=err_df["PC2"],
                    color=Group, 
                    symbol='Plasma source',
                    color_discrete_map=colour_dict,
                    symbol_map=symbol_dict, 
...
#Time trajectory
traject_df = df2_scores_.copy()
traject_df.sort_values(by=['Group'], inplace=True, key=lambda x: x.map({"CR + INT777 (L)": 0, "CR + INT777 (H)": 1, "sham ad libitum": 2}))
traject_df['Plasma source'] = meta['Plasma source']
med_df = traject_df.groupby([ Group, 'Plasma source']).median()
err_df = traject_df.groupby([Group, 'Plasma source']).sem()

med_df = med_df.reset_index()
err_df = err_df.reset_index()


med_df.sort_values(by=['Plasma source'], inplace=True, key=lambda x: x.map({"Portal vein": 0, "Systemic": 1}))
med_df.sort_values(by=['Group'], inplace=True, key=lambda x: x.map({"CR + INT777 (L)": 0, "CR + INT777 (H)": 1, "sham ad libitum": 2}))
med_df.reset_index(drop=True, inplace=True)

err_df.sort_values(by=['Plasma source'], inplace=True, key=lambda x: x.map({"Portal vein": 0, "Systemic": 1}))
err_df.sort_values(by=['Group'], inplace=True, key=lambda x: x.map({"CR + INT777 (L)": 0, "CR + INT777 (H)": 1, "sham ad libitum": 2}))
err_df.reset_index(drop=True, inplace=True)
...
# create a new trace for the connecting line
fig.add_trace(go.Scatter(
    x=med_df.loc[0:2, "PC1"], # x-coordinates of the line
    y=med_df.loc[0:2, "PC2"], # y-coordinates of the line
    mode='lines', # specify the trace type as lines
    line=dict(color=colour1, width=2), # set the color and width of the line
    showlegend=False # hide the trace from the legend
))
...
# create a new trace for the connecting line
fig.add_trace(go.Scatter(
    x=med_df.loc[3:5, "PC1"], # x-coordinates of the line
    y=med_df.loc[3:5, "PC2"], # y-coordinates of the line
    mode='lines', # specify the trace type as lines
    line=dict(color=colour2, width=2), # set the color and width of the line
    showlegend=False # hide the trace from the legend
))
...
# create a new trace for the connecting line
fig.add_trace(go.Scatter(
    x=med_df.loc[6:, "PC1"], # x-coordinates of the line
    y=med_df.loc[4:6, "PC2"], # y-coordinates of the line
    mode='lines', # specify the trace type as lines
    line=dict(color=colour3, width=2), # set the color and width of the line
    showlegend=False # hide the trace from the legend
))
