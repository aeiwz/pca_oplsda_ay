...
dataset =dataset.drop(dataset[dataset['post op AF'] == 'No'].index)
...
for f in project:
    ...
    #Make directory
    # path folder
    path_ = '/Volumes/CAS9/Aeiwz/Project/KOI_CC/Report/AF_yes/{}'.format(f)
    ...
    for i in range(len(c_list)):
        ...
        df_opls_scores = pd.DataFrame({'t_scores': cv.scores, 't_ortho': cv.orthogonal_score, 't_pred': cv.predictive_score, 'label': meta["Cardioplegia"]})
